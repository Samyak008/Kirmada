...
class GDriveStorage:
    ...
    def authenticate(self):
        ...
        # ADDED: Ensure token directory exists
        token_dir = os.path.dirname(self.token_path)
        if token_dir and not os.path.exists(token_dir):
            print(f"DEBUG: Creating token directory: {token_dir}")
            os.makedirs(token_dir, exist_ok=True)
        
        # Load existing token if available
        if os.path.exists(self.token_path):
            print(f"DEBUG: Loading existing token from {self.token_path}")
        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                # ADDED: Check if we're already in the middle of OAuth flow
                with self._lock:
                    # Check again if token was created by another thread
                    if os.path.exists(self.token_path):
                        print("DEBUG: Token was created by another thread, loading it")
                        creds = Credentials.from_authorized_user_file(self.token_path, self.SCOPES)
                    else:
                        ...
                        print(f"DEBUG: Saving token to {self.token_path}")
                        with open(self.token_path, 'w') as token:
                            token.write(creds.to_json())
            elif creds.valid:
                # Save refreshed credentials
                print(f"DEBUG: Saving refreshed token to {self.token_path}")
                with open(self.token_path, 'w') as token:
                    token.write(creds.to_json())
        ...
        for path in possible_paths:
            if os.path.exists(path):
                try:
                    ...
                    logger.info(f"Loaded {len(self.folder_ids)} folder IDs from {path}")
                    return
                except Exception as e:
                    logger.warning(f"Failed to load from {path}: {e}")
        
        logger.warning(f"Folder IDs file not found in any of: {possible_paths}")
