...
from agents.video_prompt_generation_agent import VideoPromptGenerationAgent
...
- Understand the user request completely
- Determine if research, content creation, or other functions are needed
- Plan the sequence of operations based on the user's goal
- Set up context tracking for the entire workflow

**PHASE 2 - RESEARCH & DISCOVERY:**
1. Execute comprehensive search for trending tech news using the Search Function
...
**PHASE 3 - SELECTION & EXTRACTION:**
3. Wait for user selection of articles to process
4. Once user selects, activate Crawl Function to extract full content and images
5. Retrieve complete article text, metadata, and image URLs
6. **AUTOMATICALLY store all crawled content and images in Supabase database**

**PHASE 4 - CONTENT GENERATION:**
7. Generate viral script optimized for selected platform
8. Create engaging hooks, structured content, and clear CTAs
9. Optimize for platform-specific requirements and timing

**PHASE 5 - ASSET CREATION:**
10. Generate image prompts based on the script
11. Create images using AI image generation tools
12. Generate video prompts for motion content
13. Create voiceover if requested

**PHASE 6 - ASSEMBLY & VALIDATION:**
14. Assemble all components (text, images, video, voice) into final content
15. Validate content quality and compliance
16. Store final content in appropriate locations
17. Provide organized output for user

📋 **WORKFLOW MANAGEMENT:**

- **Decompose complex requests** into smaller, manageable tasks
- **Monitor progress** and handle errors gracefully
- **Maintain context** across all phases of the workflow
- **Adapt approach** based on user feedback and changing requirements
- **Optimize for scale** by caching results and avoiding redundant operations
- **Track quality metrics** to improve output over time
- **Plan ahead** by anticipating next steps in the workflow
...
async def process_prompts_to_images(prompts: Union[List[Dict], List[str]], platform: str = "youtube") -> Dict:
    ...
    from agents.image_generation_agent import check_image_generation_status
    ...
    for i, prompt in enumerate(prompts):
        ...
        # Generate the image
        from agents.image_generation_agent import generate_image_flux
...
async def generate_visuals_from_timing(visual_timing: Union[Dict, str], platform: str = "youtube") -> Dict:
    ...
    if status.get("status") != "available":
        ...
        from agents.image_generation_agent import extract_visual_cues_from_timing
    ...
    try:
        ...
        from agents.image_generation_agent import generate_from_visual_timing
...
async def process_selection(urls_to_crawl: list, platform: str):
    ...
    if len(urls_to_crawl) == 1:
        ...
        from agents.crawl_agent import crawl_article_content
        ...
        from agents.supabase_agent import store_article_content_sync_wrapped
        ...
        from agents.scripting_agent import generate_viral_script
        ...
        try:
            from agents.visual_agent import generate_visual_timing
            ...
            try:
                from agents.voice_generation_agent import generate_voiceover
                ...
                if "VOICEOVER GENERATED SUCCESSFULLY" in voice_result:
                    ...
                    if voice_file_path:
                        try:
                            from utils.gdrive_storage import initialize_gdrive_storage
            ...
            from agents.supabase_agent import store_script_content
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Option to run the advanced workflow with backend logic
        print("\n" + "="*50)
        print("Running Advanced Workflow with Backend Logic")
        print("="*50)
        from advanced_agent import create_advanced_workflow
