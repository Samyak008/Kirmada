...
class ResearchAgent(SpecializedAgent):
    """Research agent implementation with search and crawl capabilities"""
...
Return ONLY the JSON array with no additional text."""
    ...
    def _execute_task(self, task: Task, agent_state: AgentState) -> Dict[str, Any]:
        """Execute research tasks with async support"""
        import asyncio

        async def execute_async():
            if task.task_type == "search":
                return await self._intelligent_search(agent_state.content_request)
            elif task.task_type == "crawl":
                # Get URLs from previous search results
                urls = []
                if agent_state.research_data and agent_state.research_data.sources:
                    urls = [source.url for source in agent_state.research_data.sources[:5]]  # Limit to 5 URLs
    ...
    def _update_agent_state(self, agent_state: AgentState, result: Dict[str, Any]) -> None:
        ...
        if result.get("success", True) and "results" in result:
            # Search results
            sources = []
            ...
            research_data = ResearchData(
                ...
                sources=sources,
                key_findings=[item.get("relevance_reason", "") for item in result["results"][:3]],
                research_notes=f"Found {len(sources)} relevant sources"
            )

            agent_state.research_data = research_data

        elif "crawl_results" in result:
            # Crawl results - update existing sources with full content
            if agent_state.research_data:
                for crawl_result in result["crawl_results"]:
                    if crawl_result["success"]:
                        # Find matching source and update content
                        for source in agent_state.research_data.sources:
                            if source.url == crawl_result["url"]:
